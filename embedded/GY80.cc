...
GY80::GY80()
{
   ...
   //a_scale = 0;
   g_scale = 1;
   //m_scale_z = 1;//처음에는 g_scale_z였다.헤더에도 없는거 보니 m scale z라고 판단 하였다.
   m_scale_values[0] = 0;
   m_scale_values[1] = 0.0909090909090909;
   m_scale_values[2] = 0.1169590643274854;
   m_scale_values[3] = 0.1492537313432836;
   m_scale_values[4] = 0.2222222222222222;
   m_scale_values[5] = 0.25;
   m_scale_values[6] = 0.303030303030303;
   m_scale_values[7] = 0.4347826086956522;
   m_scale_values_z[0] = 0;
   m_scale_values_z[1] = (float)100 / 980;
   m_scale_values_z[2] = (float)100 / 760;
   m_scale_values_z[3] = (float)100 / 600;
   m_scale_values_z[4] = (float)100 / 400;
   m_scale_values_z[5] = (float)100 / 355;
   m_scale_values_z[6] = (float)100 / 295;
   m_scale_values_z[7] = (float)100 / 205;
...
GY80_base_scaled GY80::cal_gryo_avg()
{
   GY80_scaled scale;
   GY80_base_scaled b_scale;
   GY80_single_scaled gryo_sum_scale, acc_sum_scale;

   for (int i = 0; i < AVGCNT; i++) {
      scale = read_scaled();

      acc_sum_scale.x += scale.a_x;
      acc_sum_scale.y += scale.a_y;
      acc_sum_scale.z += scale.a_z;

      gryo_sum_scale.x += scale.g_x;
      gryo_sum_scale.y += scale.g_y;
      gryo_sum_scale.z += scale.g_z;

      delay(100);
   }

   b_scale.b_a_x = acc_sum_scale.x / AVGCNT;
   b_scale.b_a_y = acc_sum_scale.y / AVGCNT;
   b_scale.b_a_z = acc_sum_scale.z / AVGCNT;

   b_scale.b_g_x = gryo_sum_scale.x / AVGCNT;
   b_scale.b_g_y = gryo_sum_scale.y / AVGCNT;
   b_scale.b_g_z = gryo_sum_scale.z / AVGCNT;

   return b_scale;
}

unsigned long GY80::init_time()
{
   return millis();
}

GY80_time_scale GY80::cal_time(unsigned long start)
{
   GY80_time_scale t_scale;
   t_scale.t_start = start;
   t_scale.t_end = millis();

   t_scale.t_diff = (t_scale.t_end - t_scale.t_start) / 1000.0;
   
   t_scale.t_start = t_scale.t_end;
